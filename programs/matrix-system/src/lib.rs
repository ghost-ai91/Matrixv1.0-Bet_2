...
declare_id!("CoRsyf6xCdgfKp64dG5oEKgRZLn5ckrux5YyCLoQ9rk4");
...
/// Fails transaction for ANY error (reading data OR calculation overflows)
/// Returns calculated amount to calling function which handles final fallbacks
fn get_donut_tokens_amount<'info>(
    ...
    // 1. Read LP token values - FAIL TRANSACTION IF ERROR
    let a_vault_lp_amount: u64;
    let b_vault_lp_amount: u64;
    
    {
        // Deserialize the LP token accounts - MUST SUCCEED OR FAIL
    ...
    // 2. Read total LP token supplies - FAIL TRANSACTION IF ERROR
    ...
    // 3. Read total token values in the vaults - FAIL TRANSACTION IF ERROR
    ...
    // 4. Check for zero values - FAIL TRANSACTION IF POOL IS EMPTY
    ...
    // 6. Calculate token quantities in the pool - FAIL TRANSACTION IF OVERFLOW
    ...
    let pool_token_b = match token_b_big.checked_mul(b_lp_amount_big) {
        Some(num) => match num.checked_div(b_lp_supply_big) {
            Some(result) => result,
            None => {
                msg!("Division overflow in pool_token_b calculation");
                return Err(error!(ErrorCode::MeteoraCalculationOverflow));
            }
        },
        None => {
            msg!("Multiplication overflow in pool_token_b calculation");
            return Err(error!(ErrorCode::MeteoraCalculationOverflow));
        }
    };
    ...
    // 7. Check for zero values after calculation - FAIL TRANSACTION
    if pool_token_a == 0 || pool_token_b == 0 {
        msg!("Calculated pool tokens are zero");
        return Err(error!(ErrorCode::MeteoraCalculationOverflow));
    }
    
    // 8. Calculate the basic ratio - FAIL TRANSACTION IF OVERFLOW
    ...
    msg!("Basic ratio (scaled): {}", basic_ratio);
    
    // 9. Calculate tokens directly with the basic ratio - FAIL TRANSACTION IF OVERFLOW
    ...
    // 10. Remove precision factor - FAIL TRANSACTION IF OVERFLOW
    ...
    // 11. Check for overflow to u64 - FAIL TRANSACTION IF OVERFLOW
    ...
    // 13. Handle any zero result - simplified logic
    if donut_tokens == 0 {
        msg!("Zero or small calculated value, returning minimum");
        return Ok(1); // 0.000000001 DONUT (smallest possible value)
    }
    
    // Return the calculated amount - calling function handles any final fallbacks
    Ok(donut_tokens)
}
